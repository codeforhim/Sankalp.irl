...
# Assuming the Node backend is reachable at http://backend:5000 in Docker
...
@tool
def create_complaint(user_id: int, issue_type: str, latitude: float, longitude: float, description: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"error": f"Failed to submit complaint: {str(e)}"}

@tool
def get_ward_complaints(ward_id: int) -> List[Dict[str, Any]]:
...
@tool
def update_complaint_status(complaint_id: int, status: str) -> Dict[str, Any]:
    ...
    try:
        ...
        response.raise_for_status()
        return {"success": True, "message": f"Complaint {complaint_id} updated to {status}."}
    except Exception as e:
        return {"error": str(e)}

@tool
def get_priority_complaints(ward_id: int) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        if isinstance(data, list):
            ...
            return sorted([c for c in data if c.get("status") != "resolved"], key=lambda x: x.get("priority_score", 0), reverse=True)[:5]
        return [{"error": "Unexpected format"}]
    except Exception as e:
        return [{"error": str(e)}]

@tool
def get_heatmap_insights() -> Dict[str, Any]:
    ...
    try:
        ...
        response.raise_for_status()
        data = response.json()
        return {"total_hotspots_tracked": len(data), "sample": data[:5]}
    except Exception as e:
        return {"error": str(e)}

@tool
def get_public_updates() -> List[Dict[str, Any]]:
