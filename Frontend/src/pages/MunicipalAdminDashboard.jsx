        ...
        <div className="space-y-5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="md:col-span-1 space-y-2">
                    ...
                    <div className="bg-[#F5F7FA] rounded-xl p-3 border border-[#E5E7EB]">
                        ...
                        <input
                            ...
                            className="w-full accent-[#138808]"
                        />
                ...
                <div className="space-y-3">
                    ...
                    {loading && (
                        <p className="text-xs text-[#9CA3AF] text-center">This may take 30-60 seconds for the causal ML model to run...</p>
                    )}
                </div>
            </div>
            ...
            {/* Error */}
            {error && (
                <div className="bg-red-50 border border-[#DC2626]/20 rounded-xl p-4 text-[#DC2626] text-sm">
                    ⚠️ {error}
                </div>
            )}
            ...
            {result && (
                <div className="space-y-4 animate-fadeIn">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                        {[
                            { label: 'Total Budget Used', value: formatCurrency(result.stats?.basic?.allocated_budget || 0), icon: DollarSign, color: 'text-[#138808]', bg: 'bg-green-50', borderColor: 'border-t-[#138808]' },
                            ...
                            { label: 'ROI per Rupee', value: (result.stats?.basic?.roi_uplift_per_rupee || 0).toFixed(6), icon: Activity, color: 'text-amber-600', bg: 'bg-amber-50', borderColor: 'border-t-amber-500' },
                        ].map(kpi => (
                    ...
                    {topWards.length > 0 && (
                        <div className="bg-white rounded-xl p-4 border border-[#E5E7EB] shadow-sm">
                            <h3 className="text-sm font-bold text-[#1F2937] mb-4 flex items-center gap-2">
                                <BarChart2 className="w-4 h-4 text-[#1B3A6F]" />
                                ...
                                {topWards.map(([wardName, data]) => (
                                    <div key={wardName} className="flex items-center gap-3">
                                        ...
                                        <div className="flex-1 bg-[#EEF2F7] rounded-full h-5 overflow-hidden">
                                            <div
                                                className="h-full bg-gradient-to-r from-[#1B3A6F] to-[#FF9933] rounded-full flex items-center px-2 transition-all duration-700"
                    ...
                    {result.allocations?.length > 0 && (
                        <div className="bg-white rounded-xl border border-[#E5E7EB] shadow-sm overflow-hidden">
                            <div className="px-4 py-3 border-b border-[#E5E7EB]">
                                <h3 className="text-sm font-bold text-[#1F2937]">Sample Beneficiary Allocations (Top 20)</h3>
                            </div>
                                    ...
                                    <tbody>
                                        {result.allocations.slice(0, 20).map((row, idx) => (
                                            <tr key={idx} className={`border-t border-[#E5E7EB] ${idx % 2 === 0 ? 'bg-white' : 'bg-[#F5F7FA]'} hover:bg-[#EEF2F7]`}>
                                                ...
                                                <td className="px-4 py-2 text-right text-[#138808] font-mono text-xs">
                                                    ₹{Math.round(row.cost || 0).toLocaleString('en-IN')}
                                                </td>
                                                <td className="px-4 py-2 text-right text-[#FF7A00] font-mono text-xs">
                ...
                {[
                    ...
                    <button
                        ...
                        className={`flex items-center gap-2 px-4 py-2.5 rounded-t-xl text-sm font-semibold border-b-2 transition -mb-px ${
                            activeTab === tab.id
                                ? 'border-[#1B3A6F] text-[#1B3A6F] bg-[#1B3A6F]/5'
                                ...
                                <div key={complaint.id} className="bg-[#F5F7FA] rounded-xl border border-[#DC2626]/20 p-4">
                                    <div className="flex justify-between items-start mb-3">
                                        ...
                                        <span className={`text-xs px-2 py-1 rounded font-bold uppercase border ${
                                            ...
                                            ? 'bg-red-50 text-[#DC2626] border-[#DC2626]/20' 
                                            : 'bg-green-50 text-[#138808] border-[#138808]/20'
                                        }`}>
                                    ...
                                    <div className="flex gap-4 mb-4">
                                        <div className="flex-1 text-center">
                                            <p className="text-xs font-semibold text-[#9CA3AF] mb-1 tracking-wider uppercase">Citizen "Before"</p>
                                        ...
                                        <div className="flex-1 text-center">
                                            <p className="text-xs font-semibold text-[#DC2626] mb-1 tracking-wider uppercase">Officer "After"</p>
                                            <div className="h-24 bg-[#EEF2F7] rounded-lg overflow-hidden border border-[#DC2626]/20 flex items-center justify-center relative">
                                                {complaint.after_image_url ? (
                                                    ...
                                                    <span className="text-xs font-bold text-[#DC2626] text-center px-1">AI PROCESSING...</span>
                                                )}
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    {complaint.ai_feedback && (
                                        <div className="bg-red-50 border border-[#DC2626]/10 rounded-lg p-3 mt-2">
                                            <p className="text-[10px] font-bold text-[#DC2626] uppercase tracking-tight mb-1">AI Reasoning Assessment:</p>
                                            <p className="text-xs text-[#1F2937] italic font-medium leading-relaxed">"{complaint.ai_feedback}"</p>
                                        </div>
                                    )}
            ...
            {activeTab === 'welfare' && (
                <div className="bg-white rounded-2xl border border-[#1B3A6F]/20 p-5 shadow-sm">
                    <div className="flex justify-between items-center border-b border-[#1B3A6F]/10 pb-3 mb-5">
                        ...
                        <span className="text-xs bg-[#1B3A6F]/10 text-[#1B3A6F] px-3 py-1 rounded-lg border border-[#1B3A6F]/20">Feature 2 Active</span>
                    </div>
                    <WelfareOptimizer />
                </div>
            )}
            ...
            {activeTab === 'leaderboard' && (
                <div className="bg-white rounded-2xl border border-[#E5E7EB] shadow-sm p-5">
                    <h2 className="text-lg font-semibold text-[#1F2937] border-b border-[#E5E7EB] pb-3 mb-4 flex items-center">
                        <Award className="w-5 h-5 mr-2 text-[#FF9933]" />
                        Ward Performance Leaderboard
                    </h2>
                    <div className="space-y-4">
                        {[
                            { rank: 1, name: 'Ward 12 (Central)', speed: '4.2h', sentiment: '88%', status: 'Excellent', statusStyle: 'bg-green-50 text-[#138808] border-[#138808]/20' },
                            { rank: 2, name: 'Ward 5 (North)', speed: '6.5h', sentiment: '79%', status: 'Good', statusStyle: 'bg-blue-50 text-[#1B3A6F] border-[#1B3A6F]/20' },
                            { rank: 14, name: 'Ward 8 (East)', speed: '48h', sentiment: '42%', status: 'Needs Attention', statusStyle: 'bg-red-50 text-[#DC2626] border-[#DC2626]/20' },
                        ].map(w => (
                            <div key={w.rank} className="flex justify-between items-center border-b border-[#E5E7EB] pb-3">
                                <div className="flex items-center">
                                    <span className={`w-8 h-8 flex items-center justify-center font-bold text-sm rounded-full mr-3 ${w.rank <= 3 ? 'bg-[#FF9933]/10 text-[#FF7A00] border border-[#FF9933]/20' : 'bg-[#F5F7FA] text-[#6B7280] border border-[#E5E7EB]'}`}>{w.rank}</span>
                                    <div>
                                        <p className="font-semibold text-[#1F2937]">{w.name}</p>
                                        <p className="text-xs text-[#9CA3AF]">Speed: {w.speed} • Sentiment: {w.sentiment}</p>
                                    </div>
                                </div>
                                <span className={`${w.statusStyle} border px-3 py-1 rounded-full text-xs font-bold`}>{w.status}</span>
                            </div>
                        ))}
                    </div>
                </div>
