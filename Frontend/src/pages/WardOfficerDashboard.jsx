...
import { Map, Image as ImageIcon, Camera, CheckCircle, Sparkles, RefreshCw } from 'lucide-react';
...
const WardSummaryCard = ({ wardId }) => {
    ...
    return (
        <div className="flex flex-col flex-1 space-y-3">
            {summary ? (
                <>
                    <div className="bg-[#1B3A6F]/5 border rounded-xl p-4">
                        <p className="text-sm text-[#1B3A6F]">🤖 {summary}</p>
                    </div>

                    {stats && (
                        <div className="grid grid-cols-3 md:grid-cols-6 gap-2">
                            {Object.entries(stats).map(([key, val]) => (
                                <div key={key} className="bg-gray-50 border rounded-lg p-2 text-center">
